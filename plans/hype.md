...
Fullscreen presentation uses arrows to navigate, Space for video playback, and Escape to return to editing. Stop video when leaving its slide. On a multi-display desktop, a separate presenter window shows the current slide, next slide, and notes from HTML comments on the other display.
